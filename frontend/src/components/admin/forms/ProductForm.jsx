...
const ProductForm = ({ product, onSubmit, onCancel, isEditing = false }) => {
  ...
  useEffect(() => {
    if (product && isEditing) {
      ...
      if (product.salePrice) setValue('salePrice', product.salePrice.toString());
      setValue('stock', product.stock.toString());
    }
  }, [product, isEditing, setValue]);

  const handleImageUpload = (e) => {
    // In a real implementation, you would handle file uploads to a server
    // For demonstration, we'll just use the file names
    const files = Array.from(e.target.files);
    const currentImages = watchedImages || [];
    setValue('images', [...currentImages, ...files.map(file => URL.createObjectURL(file))]);
  };
  ...
  const addTag = () => {
    if (newTag.trim() && (!watchedTags || !watchedTags.includes(newTag.trim()))) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full p-2 pl-7 border rounded-md ${errors.price ? 'border-red-500' : 'border-gray-300'}`}
                  />
                </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    {...register("salePrice", { 
                      validate: value => !value || !isNaN(parseFloat(value)) || "Sale price must be a number"
                    })}
                    className={`w-full p-2 pl-7 border rounded-md ${errors.salePrice ? 'border-red-500' : 'border-gray-300'}`}
                  />
                </div>
              ...
              <div>
                ...
                <input
                  {...register("stock", { 
                    ...
                    validate: value => !isNaN(parseInt(value)) || "Stock must be a number"
                  })}
          ...
          <div>
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                {watchedTags && watchedTags.map((tag, index) => (
                  <div 
                    key={index} 
            ...
            <div className="mb-4">
              ...
              <div className="grid grid-cols-4 gap-2">
                {['Red', 'Black', 'White', 'Blue', 'Green', 'Yellow', 'Brown', 'Pink'].map((color) => (
