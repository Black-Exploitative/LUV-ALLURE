...
const FilterSortBar = ({ onGridChange }) => {
        ...
        {activeFilters.map((filter) => {
          if (filter.type === "price") {
            ...
            return filter.values.map((value) => (
              ...
              >
                <span className="mr-1">
                  {filter.type.charAt(0).toUpperCase() + filter.type.slice(1)}: {value}
                </span>
                <IoCloseOutline
                  className="cursor-pointer"
  ...
  const filterOptions = {
    colour: ["Black", "White", "Red", "Blue", "Green", "Beige", "Pink", "Yellow"],
    size: ["XS", "S", "M", "L", "XL"],
        ...
        {filterOptions.colour.map((color) => (
          <button
            ...
            className={`flex flex-col items-center ${
              selectedFilters.colour.includes(color) ? "opacity-100" : "opacity-70"
            }`}
            onClick={() => toggleSelection("colour", color)}
          >
            <div 
              className={`w-8 h-8 rounded-full mb-1 ${
                selectedFilters.colour.includes(color) ? "ring-2 ring-black" : ""
              }`} 
              style={{ 
  ...
  const renderFilterSection = (title, type, content) => {
    ...
    return (
      <div className="border-b border-gray-200">
        ...
        >
          <h4 className="text-sm font-medium uppercase tracking-wide">{title}</h4>
    ...
    <div className="mb-6">
      <div className="flex items-center justify-between p-4 border-b border-gray-300">
        <div className="flex items-center">
          ...
          >
            <span className="tracking-wider text-sm uppercase font-light">Filter</span>
            ...
            <div className="flex flex-col h-full">
              <div className="p-6 border-b border-gray-200">
                <div className="flex justify-between items-center">
                  <h3 className="text-sm font-light uppercase tracking-widest">Filter</h3>
                  <button
                    onClick={toggleFilter}
              ...
              <div className="flex-1 overflow-y-auto">
                <div className="p-6">
                  {renderFilterSection("Colour", "colour", renderColorOptions())}
                  
                  {renderFilterSection("Size", "size", renderButtonOptions("size"))}
                  
                  {renderFilterSection("Length", "length", renderButtonOptions("length"))}
                  
                  {renderFilterSection("Style", "style", renderButtonOptions("style"))}
                  
                  {renderFilterSection("Occasion", "occasion", renderButtonOptions("occasion"))}
                  
                  {renderFilterSection("Category", "category", renderButtonOptions("category"))}
                  
                  {renderFilterSection("Features", "features", renderButtonOptions("features"))}
                  
                  {renderFilterSection("Collection", "collection", renderButtonOptions("collection"))}
                  
                  {renderFilterSection("Fabric", "fabric", renderButtonOptions("fabric"))}
                    ...
                    <div className="px-1 mt-3">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setSelectedFilters({
                            ...selectedFilters,
                            price: [selectedFilters.price[0], parseInt(e.target.value)]
                          })
                        }
                        className="w-full accent-black"
                      />
                      <div className="mt-4 mb-2 text-xs text-gray-800 font-medium">Range:</div>
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) =>
                            setSelectedFilters({
                              ...selectedFilters,
                              price: [parseInt(e.target.value), selectedFilters.price[1]]
                            })
                          }
