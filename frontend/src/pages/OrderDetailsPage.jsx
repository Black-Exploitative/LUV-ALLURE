...
const OrderDetails = () => {
    ...
    const fetchOrderDetails = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          // Format prices to ensure consistency
          const orderData = response.data.order;

          // Check if prices need formatting
          if (
            typeof orderData.subtotal === "number" &&
            orderData.subtotal > 1000000
          ) {
            orderData.subtotal = orderData.subtotal / 100;
            orderData.tax = orderData.tax / 100;
            orderData.shipping = orderData.shipping / 100;
            orderData.total = orderData.total / 100;

            if (orderData.packagingDetails?.packagingPrice) {
              orderData.packagingDetails.packagingPrice =
                orderData.packagingDetails.packagingPrice / 100;
            }

            // Also fix item prices if needed
            if (orderData.items && orderData.items.length > 0) {
              orderData.items = orderData.items.map((item) => {
                const price = parseFloat(item.price);
                return {
                  ...item,
                  price: price > 10000 ? (price / 100).toString() : item.price,
                };
              });
            }
          }
  ...
  const getTrackingUrl = (order) => {
    ...
    if (order.shippingProvider === "Bolt") {
      // Bolt tracking URL
      return (
        order.trackingUrl || `https://bolt.eu/tracking/${order.trackingNumber}`
      );
    } else if (order.shippingProvider === "GIGL") {
      // GIGL tracking URL
      return (
        order.trackingUrl ||
        `https://giglogistics.com/tracking?code=${order.trackingNumber}`
      );
    }
        ...
        <div className="bg-white rounded-lg shadow-sm overflow-hidden">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {order.status === "completed"
                    ...
                    : `Estimated delivery: ${
                        order.estimatedDeliveryDays || "3-5"
                      } business days`}
                </p>
          ...
          <div className="p-6 border-b border-gray-200">
            ...
            {order.packagingDetails &&
              order.packagingDetails.packagingType !== "normal" && (
